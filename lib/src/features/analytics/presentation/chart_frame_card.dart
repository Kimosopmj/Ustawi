...
import 'package:ustawi/src/features/analytics/presentation/chart_frame_controller.dart';
import 'package:ustawi/src/features/analytics/presentation/mood_count_bar_chart.dart';
import 'package:ustawi/src/features/analytics/presentation/mood_variation_line_chart.dart';
...
class _ChartFrameState extends ConsumerState<ChartFrame> {
  ...
  Widget build(BuildContext context) {
    final controller =
        ref.watch(chartFrameControllerProvider(_dropdownValue).notifier);
        ...
        child: Column(
          children: [
            ...
            ValueListenableBuilder(
              valueListenable:
                  controller.moodRecordsRepository.box.listenable(),
  ...
  Widget buildChart(ChartType type, List<MoodRecord> moodRecords) {
    return switch (type) {
      ...
      ChartType.moodVariation =>
        MoodVariationLineChart(moodRecords: moodRecords),
    };
  }
}
