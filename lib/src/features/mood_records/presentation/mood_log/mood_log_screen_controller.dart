...
class MoodRecordsScreenController extends Notifier<MoodRecordRepository> {
  ...
  SplayTreeMap<String, List<MapEntry<dynamic, MoodRecord>>>
      groupMoodRecordsByDay(Map<dynamic, MoodRecord> boxMap) {
    // final records = _repository.box.toMap().entries;
    final groups = groupBy(boxMap.entries,
        (record) => DateFormat("yyyy-MM-dd").format(record.value.date));
...
final moodRecordScreenControllerProvider =
    NotifierProvider<MoodRecordsScreenController, MoodRecordRepository>(
        MoodRecordsScreenController.new);
